    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    '忠告<br>これはあなたのこのランチャーでログインした最後のアカウントです。',
                    'ログアウトした後再びログインしなければいけません<br><br>本当にログインしてもいいですか?',
                    'ログアウト',
                    'キャンセル'
                )
                setOverlayHandler(() => {
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? '選択済み>選択したアカウント &#10004;' : '>このアカウントへ変更する。'}</button>
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'プレスリリース'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = '安定リリース'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/emeteam/NekominLauncher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `あたらしい ${isPrerelease(data.version) ? 'プレスリリース' : 'リリース'} があります！`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('github経由からダウンロードしてください<span style="font-size: 10px;color: gray;text-shadow: none !important;">Close the launcher and run the dmg to update.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = 'あなたは最新のバージョンのランチャーを見ています。'
